...
const adminControllers = {
  admin: async (req, res) => {
    try {
      ...
      res.render('pages/admin/admin', { items: productos, page, totalPages, header: 'admin' })
    } catch (error) {
      console.log(error)
  ...
  adminCreate: async (req, res) => {
    ...
    res.render('pages/admin/create', { categorias, licencias, header: 'admin' })
  },
  store: async (req, res) => {
  ...
  adminEditUpdate: async (req, res) => {
    console.log('\n------- Ruta /admin/edit/x PUT --------')
    console.log('req.body: ' + JSON.stringify(req.body) + '\nreq.params: ' + JSON.stringify(req.params))
    // Verifico errores en el formulario recibido
    const errors = validationResult(req)
    if (!errors.isEmpty()) {
      try {
        const categorias = await CategoriasModel.findAll({
    ...
    // Si no hay errores paso a hacer el UPDATE
    try {
